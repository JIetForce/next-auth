...
- [ ] **Step 4: Update the e2e expectation.** `e2e/auth-session.spec.ts` covers the profile; a
  ...
  is that the existing test checks for the *absence* of extra fields and so never caught wrong static
  text.
...
- [ ] **Step 2: Record the trade-off where it will be found.** Add the note to `### Concerns` so
  ...
  audit found the current document asserting the *opposite* of the truth about session revocation.
