...
The `callbacks.redirect` pin described in § "Routes and redirects" prevents a same-origin `callbackUrl`/`redirectTo` value from steering a callback or logout destination anywhere other than `/`, including via the raw `/api/auth/signin/*` and `/api/auth/signout` routes. The Viewer allowlist limits accidental disclosure across the Server/Client boundary. Profile and Server Action checks prevent Header visibility from becoming authorization. A corrupt or expired cookie fails closed. POST Server Actions provide framework origin checking, while authorization remains explicit inside each action. Direct provider image loading avoids server-side image fetching and sends no referrer.
